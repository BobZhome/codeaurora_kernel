...
/* LGE_CHANGE
 * User (including daemon) sends any commands by writing cmd string to procfs file,
 * and check current mode by reading procfs file
 * 2012-03-30, chaeuk.lee@lge.com
 */
#ifdef CONFIG_LGE_IRDA_FACTORY
...
static int irtty_open(struct tty_struct *tty)
{
	 ...
	 */
	if (lge_irda_gpio_open(GPIO_IRDA_SW_EN, GPIO_DIRECTION_OUT, GPIO_LOW_VALUE)) {
		IRDA_DEBUG(4, "%s : level shifter open error\n", __func__);
		return -1;
	}
	lge_irda_gpio_write(GPIO_IRDA_SW_EN, GPIO_IRDA_SW_EN_ENABLE);

	if (lge_irda_gpio_open(GPIO_IRDA_PWDN, GPIO_DIRECTION_OUT, GPIO_HIGH_VALUE)) {
		IRDA_DEBUG(4, "%s : irda HW module open error\n", __func__);
		return -1;
	}
	lge_irda_gpio_write(GPIO_IRDA_PWDN, GPIO_IRDA_PWDN_ENABLE);
...
static void irtty_close(struct tty_struct *tty)
{
	 ...
	 * prior to transfering any data
	 */
	lge_irda_gpio_write(GPIO_IRDA_SW_EN, GPIO_IRDA_SW_EN_DISABLE);
	lge_irda_gpio_write(GPIO_IRDA_PWDN, GPIO_IRDA_PWDN_DISABLE);
#ifdef CONFIG_LGE_IRDA_FACTORY
	_dev = NULL;
#endif /* CONFIG_LGE_IRDA_FACTORY */
}
