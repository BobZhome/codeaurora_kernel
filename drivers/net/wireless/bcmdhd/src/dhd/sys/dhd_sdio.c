...
dhd_bus_txctl(struct dhd_bus *bus, uchar *msg, uint msglen)
{
	...
	*(((uint16*)frame) + 1) = htol16(~msglen);
	...
	if (unlikely(ret))
	{
		DHD_ERROR(("P:%s:txcnt_timeout[%d]:ctlerrs[%d]:ret[%d]\n", __FUNCTION__, bus->dhd->txcnt_timeout, (int)bus->dhd->tx_ctlerrs, ret));
	}
#endif
...
#endif
#endif /* DHD_DEBUG */
//joon for test. plz check. _S //retry once
		/* Wait until control frame is available */
#if 1
		DHD_ERROR(("J:%s: RETRY START.....\n",__func__ ));
		timeleft = dhd_os_ioctl_resp_wait(bus->dhd, &bus->rxlen, &pending);
		...
		bus->rxlen = 0;
		dhd_os_sdunlock(bus->dhd);
		DHD_ERROR(("J:%s: RETRY END....., timeleft %d rxlen %d, pending %d\n", __func__, timeleft, rxlen, pending ));
#endif
//joon for test. plz check. _E
...
#ifdef BCM4334_CHIP
// 2012.08.29 real-wifi@lge.com[wo0gi] broadcom patch: when firmware is changed, some variable is not initialized. [START]
// related issue : 1. wifi on/off, hotspot on/off test -> 'hotspot on' blocked!!
//                 2. after wifi on/off, rft shell command is failed.
#if defined(CONFIG_LGE_BCM433X_PATCH)
	bus->_srenab = FALSE;
#endif
// 2012.08.29 real-wifi@lge.com[wo0gi] broadcom patch: when firmware is changed, some variable is not initialized. [END]
