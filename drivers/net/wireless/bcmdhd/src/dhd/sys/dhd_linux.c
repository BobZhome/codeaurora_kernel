...
int op_mode = 0;
int disable_proptx = 1;  /* LGE_UPDATE, 20121004, cheolsook.lee@lge.com, remove PROP_TX option due to crash when NFS via Direct */
...
#ifdef CUSTOMER_HW11
uint ht40_disable = 0;
module_param(ht40_disable, uint, 0644);
#endif
...
#if !defined(CONFIG_COMMON_PATCH) && (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 27)) && defined(CONFIG_PM_SLEEP)
...
#if defined(WL_CFG80211) && defined(CUSTOMER_HW4)
...
#endif /* WL_CFG80211 && CUSTOMER_HW4 */
...
static int dhd_set_suspend(int value, dhd_pub_t *dhd)
{
	char iovbuf[32];
#ifndef CUSTOMER_HW4
#ifndef CONFIG_LGE_BCM433X_PATCH
	...
	DHD_ERROR(("%s: enter, up[%d] value[%d] in_suspend=[%d]\n",
		__FUNCTION__, dhd->up, value, dhd->in_suspend));

// 20120818 real-wifi@lge.com [wo0gi] kernel crash patch : set flag 1 [START]
//	wl_android_esuspend_set_flag( 1 );
// 20120818 real-wifi@lge.com [wo0gi] kernel crash patch : set flag 1 [END]
...
// 20120818 real-wifi@lge.com [wo0gi] kernel crash patch : set flag 0 [START]
//	wl_android_esuspend_set_flag( 0 );
// 20120818 real-wifi@lge.com [wo0gi] kernel crash patch : set flag 0 [END]
	return 0;
}
...
// 20120818 real-wifi@lge.com [wo0gi] kernel crash broadcom patch [START]
// : add mutex lock during wifi on/off, problem => wifi off command is sent during late_resume.
#if defined(CONFIG_LGE_BCM433X_PATCH)
	MUTEX_LOCK_STARTSTOP_SET( &dhd->pub );
#endif
// 20120818 real-wifi@lge.com [wo0gi] kernel crash broadcom patch [END]
...
// : add mutex lock during wifi on/off, problem => wifi off command is sent during late_resume.
#if defined(CONFIG_LGE_BCM433X_PATCH)
	MUTEX_UNLOCK_STARTSTOP_SET( &dhd->pub );
#endif
// 20120818 real-wifi@lge.com [wo0gi] kernel crash broadcom patch [END]
}
...
// : add mutex lock during wifi on/off, problem => wifi off command is sent during late_resume.
#if defined(CONFIG_LGE_BCM433X_PATCH)
	MUTEX_LOCK_STARTSTOP_SET_INIT( &dhd->pub );
#endif
// 20120818 real-wifi@lge.com [wo0gi] kernel crash broadcom patch [END]
...
static int dhd_preinit_proc(dhd_pub_t *dhd, int ifidx, char *name, char *value)
{
	...
	if (!strcmp(name, "country")) {
		...
		retval = dhd_wl_ioctl_cmd(dhd, WLC_SET_PM,
				&var_int, sizeof(var_int), TRUE, 0);
...
#endif
       //add band selection for 2.4G only support model.
                  ...
                  DHD_ERROR(("%s set Band=%d \n", __FUNCTION__,var_int));	
                  return dhd_wl_ioctl_cmd(dhd, WLC_SET_BAND,
                                  &var_int, sizeof(var_int), TRUE, 0);
       }
...
dhd_preinit_ioctls(dhd_pub_t *dhd)
{
	...
	if (dhd->op_mode == 4)
	{
		glom = 0;   /* glom will be set to 0 when P2P is on */
   		DHD_ERROR(("op_mode=%d, glom=%d\n",dhd->op_mode, glom));
	}
#endif
	...
	if ( ht40_disable ) {
		...
		dhd_wl_ioctl_cmd(dhd, WLC_GET_UP, (char*)&isup, sizeof(isup), FALSE, 0); 
		if (isup)
			dhd_wl_ioctl_cmd(dhd, WLC_DOWN, (char*)&isup, sizeof(isup), TRUE, 0);
		bcm_mkiovar("mimo_bw_cap", (char *)&mimo_bw_cap, 4, iovbuf, sizeof(iovbuf));
		dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0);
		if (isup)
			dhd_wl_ioctl_cmd(dhd, WLC_UP, (char*)&isup, sizeof(isup), TRUE, 0);
		ht40_disable = 0;
	}
#endif
...
#endif /* WL_CFG80211 */
//#ifdef CUSTOMER_HW4  //LGE_UPDATE, 20121018, cheolsook.lee@lge.com, changed to below line, to ignore TXFAIL by imj
...
#else
// 2012.08.31 real-wifi@lge.com[wo0gi] BLOCK: clear multicast filter forcely [START]
//#ifdef CONFIG_COMMON_PATCH
//	dhd->pktfilter_count = 5;
//	dhd->pktfilter[4] = "104 0 0 0 0xFFFFFF 0x01005E";
//	/* customer want to get IPV4 multicast packets */
//#endif
// 2012.08.31 real-wifi@lge.com[wo0gi] BLOCK: clear multicast filter forcely [END]
...
#if !(CONFIG_COMMON_PATCH) && (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 27)) && defined(CONFIG_PM_SLEEP)
