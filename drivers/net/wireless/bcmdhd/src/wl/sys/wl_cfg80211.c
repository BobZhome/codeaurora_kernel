...
//sangjun.bae@lge.com first scan interval reduce
 ...
 */
	...
	if (act_frm && act_frm->elts) {
		if ((act_frm->subtype == P2P_PAF_PROVDIS_REQ) &&
			...
			if (wl_cfgp2p_retreive_p2pattrib(p2p_ie, P2P_SEID_GROUP_ID)) {
...
#if defined(CONFIG_LGE_BCM433X_PATCH) // to support connection with hidden SSID in mobile hotspot
...
#if defined(CONFIG_DHD_USE_STATIC_BUF) && defined(CUSTOMER_HW4)
	...
	if (status == WLC_E_STATUS_PARTIAL) {
		...
		if (!(wl_to_wiphy(wl)->interface_modes & BIT(NL80211_IFTYPE_ADHOC))) {
			if (dtoh16(bi->capability) & DOT11_CAP_IBSS) {
				WL_ERR(("P:Ignoring IBSS result:CAP[0x%X]:MAC[%.2X:%.2X:%.2X:%.2X:%.2X:%.2X]\n", dtoh16(bi->capability), bi->BSSID.octet[0], bi->BSSID.octet[1], bi->BSSID.octet[2], bi->BSSID.octet[3], bi->BSSID.octet[4], bi->BSSID.octet[5]));
				goto exit;
			}
		}
...
#ifdef CUSTOMER_HW11
				} else if (_net_info->pm_block || _net_info->pm_disabled) {
#else
...
#ifdef CUSTOMER_HW11
							if (wl_get_netinfo_by_netdev(wl, iter->ndev)->pm_block || wl_get_netinfo_by_netdev(wl, iter->ndev)->pm_disabled)
#else
