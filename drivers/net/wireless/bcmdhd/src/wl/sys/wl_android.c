...
static int wl_android_get_per(struct net_device *net, char *command, int total_len)
{
	...
	bytes_written = snprintf(command, total_len, "PER %u", per);
	DHD_INFO(("%s: command result is %s\n", __FUNCTION__, command));
	return bytes_written;
}
#endif /* CUSTOMER_HW11 */
...
{
	int ret = 0;
// 20120818 real-wifi@lge.com [wo0gi] wait until early_suspend complete [START]
#if 0
	int wait_time = 20;
	int cnt = 0;
	int wait_timeout = 0;
#endif
// 20120818 real-wifi@lge.com [wo0gi] wait until early_suspend complete [END]
...
#if 0
	while( cnt < wait_time ) {
		int flag = wl_android_esuspend_get_flag();
		if( !flag ) {
			printk(KERN_ERR "[wo0gi-dbg] system ready!!\n");
			wait_timeout = 1;
			break;
		}
		cnt++;
		printk(KERN_ERR "[wo0gi-dbg] system is not ready!! count(%d)\n", cnt);
		usleep( 100*1000 ); // wait 100ms
	}

	if( !wait_timeout ) 
		printk(KERN_ERR "[wo0gi-dbg] wait time out. 2seconds\n");
#endif
// 20120818 real-wifi@lge.com [wo0gi] wait until early_suspend complete [END]
...
int wl_android_init(void)
{
	...
	// 20120818 real-wifi@lge.com [wo0gi] add spin lock [START]
	// wl_android_esuspend_lock_init();
	// 20120818 real-wifi@lge.com [wo0gi] add spin lock [END]
...
spinlock_t lge_suspend_lock;
int lge_esuspend_flag = 0;
void wl_android_esuspend_lock_init( void )
{
	spin_lock_init(&lge_suspend_lock);	
}

void wl_android_esuspend_set_flag( int flag )
{
	printk(KERN_ERR "[wo0gi-dbg] %s>>> flag(%d)\n", __func__, flag );
	spin_lock( &lge_suspend_lock );
	lge_esuspend_flag = flag;
	spin_unlock( &lge_suspend_lock );
}   

int wl_android_esuspend_get_flag( void )
{
	return lge_esuspend_flag;
}   
//EXPORT_SYMBOL( lge_esuspend_set_flag );
//EXPORT_SYMBOL( lge_esuspend_get_flag );
#endif
// 20120818 real-wifi@lge.com [wo0gi] add spin lock [END]
