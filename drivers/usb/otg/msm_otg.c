...
// jongwook.oh@lge.com : Patch from Software Development Quality Execution Committee
// Fixed problem of USB Connection : Applied Atomic operation
atomic_t is_suspend; 
// jongwook.oh@lge.com
...
static void msm_ta_detect_work(struct work_struct *w)
{
	...
	schedule_delayed_work(&check_ta_work, delay);
}
#endif
...
static void msm_chg_detect_work(struct work_struct *w)
{
	...
	case USB_CHG_STATE_DCD_DONE:
		...
		if (vout && !dm_vlgc) { /* VDAT_REF < DM < VLGC */
			...
			if (test_bit(ID_A, &motg->inputs)) {
			dev_info(otg->dev, "charger detection done..\n");
				motg->chg_type = USB_ACA_A_CHARGER;
				motg->chg_state = USB_CHG_STATE_DETECTED;
				delay = 0;
				break;
...
#if defined(CONFIG_MACH_MSM8960_D1L_KR) || defined(CONFIG_MACH_MSM8960_L1m)
...
static void msm_otg_sm_work(struct work_struct *w)
{
			...
			case USB_CHG_STATE_DETECTED:
				...
				case USB_SDP_CHARGER:
					...
					if((boot_mode != LGE_BOOT_MODE_FACTORY) &&
							(boot_mode != LGE_BOOT_MODE_FACTORY2) &&
							(boot_mode != LGE_BOOT_MODE_PIFBOOT)) {
						schedule_delayed_work(&check_ta_work, MSM_CHECK_TA_DELAY);
					}
#endif
					break;
				default:
	...
	case OTG_STATE_A_PERIPHERAL:
		if ((test_bit(ID, &motg->inputs) &&
			...
			dev_info(otg->dev, "msm_otg_notify_charger 0\n");
			msm_otg_notify_charger(motg, 0);
			msm_hsusb_vbus_power(motg, 1);
		}
		break;
...
static irqreturn_t msm_otg_irq(int irq, void *data)
{
	...
	if ((otgsc & OTGSC_IDIS) && (otgsc & OTGSC_IDIE)) {
		...
		// jongwook.oh@lge.com : Patch from Software Development Quality Execution Committee
		// Fixed problem of USB Connection : Applied Atomic operation		
		else if(atomic_read(&is_suspend))
		{
			pr_debug("BSV clear\n");
			clear_bit(B_SESS_VLD, &motg->inputs);
			clear_bit(A_BUS_SUSPEND, &motg->inputs);
