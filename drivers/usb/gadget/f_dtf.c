...
//#define RH_DBG_MSG                      /* MSEMSEMSE */

#ifndef RH_DBG_MSG                      /* MSEMSEMSE */
#define _dbgmsg(fmt, ...)
#else                                   /* MSEMSEMSE */
#define _dbgmsg(fmt, ...)		printk(KERN_INFO "[DTF]::[%s]::[%d]::"fmt, __func__,__LINE__, ##__VA_ARGS__) /* MSEMSEMSE */
#endif  /* RH_DBG_MSG */                /* MSEMSEMSE */

//#define RH_DBG_MSG_SEQ                  /* MSEMSEMSE */

#ifndef RH_DBG_MSG_SEQ                  /* MSEMSEMSE */
#define _dbgmsg_if(fmt, ...)
#define _dbgmsg_in(fmt, ...)
#define _dbgmsg_out(fmt, ...)
#define _dbgmsg_gadget(fmt, ...)
#else                                   /* MSEMSEMSE */
#define _dbgmsg_if(fmt, ...)	printk(KERN_INFO "[DTFIF]::[%s]::[%d]::"fmt, __func__,__LINE__, ##__VA_ARGS__) /* MSEMSEMSE */
#define _dbgmsg_in(fmt, ...)	printk(KERN_INFO "[DTFIF(IN)]::[%s]::[%d]::"fmt, __func__,__LINE__, ##__VA_ARGS__) /* MSEMSEMSE */
#define _dbgmsg_out(fmt, ...)	printk(KERN_INFO "[DTFIF(OUT)]::[%s]::[%d]::"fmt, __func__,__LINE__, ##__VA_ARGS__) /* MSEMSEMSE */
#define _dbgmsg_gadget(fmt, ...)	printk(KERN_INFO "[DTFIF(GADGET)]::[%s]::[%d]::"fmt, __func__,__LINE__, ##__VA_ARGS__) /* MSEMSEMSE */
#endif  /* RH_DBG_MSG_SEQ */            /* MSEMSEMSE */
...
#define D_DTF_MINOR_NUMBER  249		//PATCH

static struct miscdevice dtf_device = {
//PATCH	.minor = MISC_DYNAMIC_MINOR,
	.minor = D_DTF_MINOR_NUMBER,	//PATCH
	.name = dtf_shortname,
	.fops = &dtf_fops,
};
...
static int dtf_allocate_interface_ids( struct usb_configuration *c, struct usb_function *f )
{
	...
	_dbgmsg_gadget( "usb_interface_id\n" );								/* MSEMSEMSE */
	id = usb_interface_id(c, f);
	_dbgmsg( "usb_interface_id() = %d(%d)\n", id, vPg1_intf_comm_desc.bInterfaceNumber );/* MSEMSEMSE */
	if( id < 0 )
		return id;
	...
	_dbgmsg( "usb_interface_id() = %d(%d)\n", id, vPg1_intf_bulk_desc.bInterfaceNumber );/* MSEMSEMSE */
