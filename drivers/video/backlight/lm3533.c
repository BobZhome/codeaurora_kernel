...
static void lm3533_set_main_current_level(struct i2c_client *client, int level)
{
	...
	if (level != 0) {
		//cal_value = (((level-50)*659)/1000)+120; //mapped_value[leve];
...
void lm3533_blue_led_set(int level)
{
/*
	if(level > 0)
	{
		lm3533_write_reg(main_lm3533_dev->client, 0x21, 0x13); // Full-Scale Current (20.2mA) BACK C
		lm3533_write_reg(main_lm3533_dev->client, 0x27, (0x04|backlight_status)); // Control Bank C is enable
		lm3533_write_reg(main_lm3533_dev->client, 0x42, level);
	}
	else if(level <= 0)
	{
		lm3533_write_reg(main_lm3533_dev->client, 0x42, 0);
	}
*/
	return;
}
EXPORT_SYMBOL(lm3533_blue_led_set);
