...
void mdp4_mixer_blend_setup(struct mdp4_overlay_pipe *pipe)
{
	...
	/*Initialyze variable with 0 for WBT issue. kiran.kanneganti@lge.com*/
	uint32 c0, c1, c2, blend_op, constant_color = 0, rgb_src_format = 0;
	...
	if (pipe->is_fg) {
		if (pipe->alpha == 0xff &&
			...
			/*We are not using below variable.Kust comment it out. WBT issue*/
	...
	struct msm_fb_data_type *mfd, uint32 perf_level, uint32 pclk_rate)
{
...
#endif
	printk("%s: LCDC Mode Downscale validation with MDP Core"
		" Clk rate\n", __func__);
	printk("src_w %u, src_h %u, dst_w %u, dst_h %u\n",
	...
	printk("hsync_period_ps %u, mdp_period_ps %u,"
	...
	printk("mdp_clks_per_hsync %u, fill_rate_y_dir %lu,"
		"fill_rate_x_dir %lu\n", mdp_clks_per_hsync,
		fill_rate_y_dir, fill_rate_x_dir);

	mdp_pixels_produced = 100 * mdp_clks_per_hsync/fillratex100;
	printk("fillratex100 %lu, mdp_pixels_produced %lu\n",
	...
	printk("check This MDP : W:%d, H:%d\n", compression_rate_w, compression_rate_h);

	if(compression_rate_w < 70 && compression_rate_w < 70)
	{

		{
			printk("Entering abnormal video downscale case Part1!!\n");
			return -ERANGE;
		}
	}
	...
	{
		printk("Entering abnormal video downscale case Part2!!\n");
#endif
		return -ERANGE;
	}
	...
	struct msm_fb_data_type *mfd, uint32 perf_level)
{
	...
	if (blt_chq_req && (req->src_rect.h > req->dst_rect.h ||
		...
		if (mdp4_overlay_validate_downscale(req, mfd, perf_level,
			clk_rate)&& mfd->panel_info.type != DTV_PANEL)
			use_blt = 1;
	}
...
/* [START] 20120208 force FLIP_MODE for INVERSE PANEL - kyunghoo.ryu@lge.com */
	...
	if (pipe->mixer_num == MDP4_MIXER2) {
		...
		if (ctrl->panel_mode & MDP4_PANEL_WRITEBACK) {
//[LGE_UPDATE_START] Stripped pattern remove during plying a video content with Miracast , kyoohyun.kim@lge.com, 20120908
		//	mdp4_writeback_dma_busy_wait(mfd);
//[LGE_UPDATE_END] Stripped pattern remove during plying a video content with Miracast , kyoohyun.kim@lge.com, 20120908
