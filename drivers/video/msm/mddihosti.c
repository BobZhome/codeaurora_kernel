...
extern boolean dev_lcd_mddi_read_status;
boolean dev_lcd_mddi_rev_err = FALSE;
...
static boolean mddi_enable_reg_read_retry = FALSE;
...
void oem_mddi_status_restoration( void )
{
...
void oem_panel_read_temporary( void )
{
  mddi_register_access_packet_type * regacc_pkt_ptr;
  uint32 data_count;
  if (mddi_reg_read_value_ptr) {
    regacc_pkt_ptr = (mddi_register_access_packet_type *)rev_packet_data;

    data_count = regacc_pkt_ptr->read_write_info & 0x3FFF;
    printk("\n MDDI rev read: 0x%x", regacc_pkt_ptr->read_write_info);
    printk("Reg Acc parse reg=0x%x,value=0x%x\n", regacc_pkt_ptr->register_address,  regacc_pkt_ptr->register_data_list[0]);

    /* Copy register value to location passed in */
    if (data_count && data_count <= MDDI_HOST_MAX_CLIENT_REG_IN_SAME_ADDR) {
      memcpy(mddi_reg_read_value_ptr, (void *)&regacc_pkt_ptr->register_data_list[0], data_count * 4);
...
static void mddi_process_rev_packets(void)
{
	...
	for (i = 0; i < rev_packet_count; i++) {
		...
		default:	/* any other packet */
			...
			if(dev_lcd_mddi_read_status)
			{
			  printk("%s() any other packet!!!!! \n" , __func__);
			  mddi_reg_read_value_ptr  = NULL;
			  mddi_reg_read_successful = TRUE;
			  dev_lcd_mddi_rev_err     = TRUE;
			}
			else
			{
				uint16 hdlr;
...
uint16 mddi_get_next_free_llist_item(mddi_host_type host_idx, boolean wait)
{
	...
	if (forced_wait) {
        wait_ret = wait_for_completion_timeout(&(pmhctl->mddi_llist_avail_comp), 1); /* 10mS */

        if(wait_ret < 0)
        {
            printk(KERN_ERR "%s: failed to wait for completion!%d \n", __func__, wait_ret);
        }
        else if(!wait_ret)
        {
            printk(KERN_ERR "%s: Timed out waiting! %d \n", __func__, wait_ret);
        }
