...
bool SiiMhlTxWritePeerRegister(uint8_t command, uint8_t offset, uint8_t value);
...
#if defined(CONFIG_MACH_MSM8960_D1LSK) || defined(CONFIG_MACH_MSM8960_D1LKT)|| defined(CONFIG_MACH_MSM8960_VU2SK)|| defined(CONFIG_MACH_MSM8960_VU2KT)
	SiiRegWrite(REG_MHLTX_CTL4, 0XE6);
...
static void InitCBusRegs (void)
{
	...
	SiiRegWrite(TX_PAGE_CBUS | 0x0036, 0x0B); 			// Drive High Time. -- changed from 0x03 on 2012-08-20 - 16:00
...
int GetMHLConnectedStatus(void)
{
	...
	if (0x02 == rgndImpedance)
	{
		printk(KERN_INFO "[MHL]MHL Device is connected.\n");
		return 1;
	}
	else
	{
		printk(KERN_INFO "[MHL]Non-MHL Device is connected.\n");
		return 0;
	}
}
	...
	switch(eventCode) {
		...
		case MHL_TX_EVENT_RCP_RECEIVED:
			...
			kobject_uevent_env(&gDriverContext.pDevice->kobj,
								KOBJ_CHANGE, envp);
			printk("[MHL_RCP] %s\n",rcp_buf);
			hdmi_common_send_uevent(rcp_buf);
			break;
...
{
	printk(KERN_INFO "[MHL]sii8334_mhl_tx_irq\n");
	...
	if(category == 0x01)
	{
		...
		if(command == 0x01)			// Magic Motion Remote Controller -> absolute position, button
		{
			...
			if(pre_r_action == r_action && pre_x_position == x_position && pre_y_position == y_position){
				printk("%s ->magic RC, double input, so remove this info. ",__func__);
				return;
			}
			else{
			...
			#else
			{
				...
				printk("%s ->magic RC, action:%d, (%d,%d) ",__func__,r_action,x_position,y_position);
		...
		{
			printk(("MhlControl : not defined message\n"));
		}
	}
...
void SiiMhlTxReadScratchpad(void)
{
	...
	for(m_idx=0; m_idx<(MHD_SCRATCHPAD_SIZE/2); m_idx++)
	{
		...
		/*printk("[MAGIC] %s, [%d] %x\n", __func__, m_idx, mhlTxConfig.mscScratchpadData[m_idx]);*/
	}
	MhlControl();
}
...
void SiiMhlTxWriteScratchpad(uint8_t *wdata)
{
	...
	for ( i = 0; i < MHD_MAX_BUFFER_SIZE; i++ )
	{
		...
		/*printk("REG_CBUS_SCRATCHPAD_0 %d /%d  => value : 0x%x\n",i,MHD_MAX_BUFFER_SIZE,wdata[i]);*/
	}
	printk("[MAGIC]%s end \n",__func__);
...
void mhl_writeburst_uevent(unsigned short mev)
{
	...
	printk("[MAGIC] mhl_writeburst_uevent %s\n",env_buf);
	return;
}
	...
	const struct i2c_device_id *id)
{
	...
	printk(KERN_INFO "[MHL]sii8334_mhl_tx_probe\n");
