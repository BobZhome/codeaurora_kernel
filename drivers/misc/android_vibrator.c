...
#define CONFIG_LGE_VIB_WR

#ifdef CONFIG_LGE_VIB_WR
...
unsigned long	flags;
	...
	if (nForce == 0) {
		...
		msm_xo_mode_vote(vib_voter, MSM_XO_MODE_OFF);
#endif
...
#ifdef CONFIG_LGE_VIB_WR
		msm_xo_mode_vote(vib_voter, MSM_XO_MODE_ON);
		spin_lock_irqsave(&vib->lock, flags);
#endif
...
#ifdef CONFIG_LGE_VIB_WR
	if(vib_voter == NULL){
		vib_voter = msm_xo_get(MSM_XO_TCXO_D0, "pmic_xoadc");
		if (IS_ERR(vib_voter)) {
			ret = PTR_ERR(vib_voter);
			ERR_MSG("failed to get the handle for D0(%d)\n", ret);
			return 0;
		}
	}
#endif
