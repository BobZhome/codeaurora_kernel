...
/*
 * pwm_config_oem - change a PWM device configuration
 *
 * @pwm: the PWM device
 * @period_us: period in micro second
 * @duty_us: duty cycle in micro second
 */
int pwm_config_oem(struct pwm_device *pwm, int duty_us)
{
	int				rc;
	int period_us = 511;/* 9bit(0-511) */
  ...
  pwm->period.pwm_size    = 9;
  pwm->period.clk         = 2;
  pwm->period.pre_div     = 0;
  pwm->period.pre_div_exp = 0;
  pwm->pwm_value          = duty_us;
