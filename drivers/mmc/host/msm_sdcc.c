...
static u32 msmsdcc_setup_pwr(struct msmsdcc_host *host, struct mmc_ios *ios)
{
	...
	case MMC_POWER_OFF:
		 ...
		 */
/* LGE_CHANGE_S [bking.moon@lge.com] 2012-11-07, */
#if 0 /* Temp Code Workaround- reduce the current without SD Card */
		msmsdcc_set_vdd_io_vol(host, VDD_IO_LOW, 0);
#else
		printk("%s: sd io pull up is high voltage so change the voltage to High Level \n", __func__);
		msmsdcc_set_vdd_io_vol(host, VDD_IO_HIGH, 0);
#endif
/* LGE_CHANGE_E [bking.moon@lge.com] 2012-11-07 */
...
//sangjun.bae@lge.com suspend/resume patch [start]
#if 0
	// sangjun.bae@lge.com - Don't process normal suspend
	if (host->mmc->index == 2)
	{
		printk(KERN_ERR "msmsdcc_runtime_suspend() - host->mmc->index=2 - Start\n");		
		if( host->clks_on ) 
			msmsdcc_setup_clocks(host, false);
		host->clks_on = 0;
		printk(KERN_ERR "msmsdcc_runtime_suspend() - host->mmc->index=2- End\n");
		return 0;
	}
#endif
//sangjun.bae@lge.com suspend/resume patch [end]
	if (host->plat->is_sdio_al_client) {
		rc = 0;
		goto out;
	}
...
//sangjun.bae@lge.com suspend/resume patch [start]
	...
	if (host->mmc->index == 2)
	{
		printk(KERN_ERR "msmsdcc_runtime_resume() - host->mmc->index=2 - Start\n");
		...
		printk(KERN_ERR "msmsdcc_runtime_resume() - host->mmc->index=2- End\n");
