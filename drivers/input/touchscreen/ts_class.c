...
ts_version_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if(lge_get_board_revno() < HW_REV_B){
		len = snprintf(buf, PAGE_SIZE, "%d\n", ts_info_cdev->version[0]);
		kfree(ts_info_cdev->version);
	}
	else
		len = snprintf(buf, PAGE_SIZE, "%s\n", ts_info_cdev->version);
#elif defined(CONFIG_MACH_MSM8960_L1v) && defined(CONFIG_LGE_TOUCH_SYNAPTICS_325)
	len = snprintf(buf, PAGE_SIZE, "%s\n", ts_info_cdev->version);
#elif defined(CONFIG_MACH_MSM8960_L1v) && defined(CONFIG_TOUCHSCREEN_MMS136)
	len = snprintf(buf, PAGE_SIZE, "%d\n", ts_info_cdev->version[0]);
	kfree(ts_info_cdev->version);
#else
	len = snprintf(buf, PAGE_SIZE, "%d\n", ts_info_cdev->version);
