...
static struct msm_camera_i2c_reg_conf s5k4e1_recommend_settings[] = {
	...
	{0x30BE, 0x15},
	{0x30C1, 0x01},
	{0x30EE, 0x12},  //MIPI setting for Twake, Tinit  0x02=>0x12
	{0x3111, 0x86},

/* MIPI setting for Twake, Tinit */
	{0x30E3, 0x38},
	{0x30E4, 0x40},
	{0x3113, 0x70},
	{0x3114, 0x80},
...
/* LGE_CHANGE_S, Apply fps value , 2012-10-08, jeongda.lee@lge.com */
	curr_fl_lines = (curr_fl_lines * s_ctrl->fps_divider) / Q10;
/* LGE_CHANGE_E, Apply fps value , 2012-10-08, jeongda.lee@lge.com */
		...
		uint16_t gain, uint32_t line)
{
	...
	msleep(50);/* need to revisit */ /*LGE_CHANGE, Fixed ANR problem during flash snapshot, 2012-10-18, kyungjin.min@lge.com */
...
/* LGE_CHANGE_S, For GPIO VCM_EN, 2012-08-19, kyungjin.min@lge.com */
#define GPIO_VCM_EN      (2)
/* LGE_CHANGE_E, For GPIO VCM_EN, 2012-08-19, kyungjin.min@lge.com */
...
/* LGE_CHANGE_S, For GPIO VCM_EN, 2012-08-19, kyungjin.min@lge.com */
	...
	pr_err("%s: Turning on VCMEN for L1m project\n", __func__);
	rc = gpio_request(GPIO_VCM_EN, "CAM_GPIO");
	if (rc) {
		pr_err("gpio_request(GPIO_VCM_EN) error\n");
	} else {
		rc = gpio_direction_output(GPIO_VCM_EN, 1);
		if (rc) {
			pr_err("gpio_direction_output(GPIO_VCM_EN) error\n");
		}
	}
#endif
/* LGE_CHANGE_E, For GPIO VCM_EN, 2012-08-19, kyungjin.min@lge.com */
...
/* LGE_CHANGE_S, For GPIO VCM_EN, 2012-08-19, kyungjin.min@lge.com */
#ifdef CONFIG_MACH_MSM8960_L1m
	pr_err("%s: Turning off VCMEN for L1m project\n", __func__);
	if (gpio_direction_output(GPIO_VCM_EN, 0)) {
		pr_err("gpio_direction_output(GPIO_VCM_EN, 0) error\n");
	} else {
