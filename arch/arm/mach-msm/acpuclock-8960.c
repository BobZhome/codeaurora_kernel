...
static struct acpu_level *acpu_freq_tbl_8960_v2_lge_factory[NUM_PVS] __initdata = {
	[PVS_SLOW] = acpu_freq_tbl_8960_kraitv2_slow_lge_factory,
	[PVS_NOM] = acpu_freq_tbl_8960_kraitv2_nom_lge_factory,
	[PVS_FAST] = acpu_freq_tbl_8960_kraitv2_fast_lge_factory,
};
#endif
...
static void __init select_freq_plan(void)
{
	...
	if (cpu_is_msm8960()) {
		...
		if(boot_mode == LGE_BOOT_MODE_FACTORY2 || boot_mode == LGE_BOOT_MODE_PIFBOOT)
		   acpu_freq_tbl = acpu_freq_tbl_8960_v2_lge_factory[pvs_id];
		else			
		#endif
