...
#if 0
	/* Updating the PPSS_PAUSE register is not needed on 8960 */
...
static int dsps_alloc_resources(struct platform_device *pdev)
{
	...
	if(smem_ramdump_dev==NULL)
		smem_ramdump_dev = create_ramdump_device("smem-dsps");
	drv->smem_ramdump_dev = smem_ramdump_dev;
#else
