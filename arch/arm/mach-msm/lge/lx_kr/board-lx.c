...
#define MSM_ION_MM_SIZE		0x7400000 /*(116MB)(168MB)(144MB)(120MB)*/ //Original MSM_PMEM_ADSP_SIZE /* LGE_CHANGE, Camera Zero shutter lag patch, 2012.01.12 jungryoul.choi@lge.com */
	...
	for (i = 0; i < msm8960_ion_pdata.nr; ++i) {
		...
		if (heap->extra_data) {
			...
			case FIXED_MIDDLE:
				...
				pdata->secure_base = fixed_middle_start- HOLE_SIZE;
...
#endif
no_dmm:
//       printk("[%s] high(0x%lx), low(0x%lx), bank_size(0x%lx)\n", __FUNCTION__, high, low, bank_size);
//20120628 ws.yang@lge.com .. because overflow of range when ram 2GB
#if defined(CONFIG_MACH_LGE)
    if (high == 0xFFFFFFFF)
        msm8960_reserve_info.low_unstable_address = high - 4096 + 1;
#else
	msm8960_reserve_info.low_unstable_address = high;
#endif
	msm8960_reserve_info.max_unstable_size = 0;
//	printk("[%s] low_unstable_address(0x%lx)\n", __FUNCTION__, msm8960_reserve_info.low_unstable_address);
