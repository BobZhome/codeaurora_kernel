...
int ts_set_vreg(int on, bool log_en)
{
	...
	if (log_en)
		printk(KERN_INFO "[Touch D] %s: power %s\n",
			__func__, on ? "On" : "Off");
	if (on)
		rc = gpio_direction_output(MSM_8960_TS_PWR_MEL, 1);
	else
		rc = gpio_direction_output(MSM_8960_TS_PWR_MEL, 0);
...
void touch_init(void)
{
	...
	if(lge_get_board_revno() >= HW_REV_B){
		...
		/* reset pin control */
		gpio_tlmm_config(GPIO_CFG(MSM_8960_TS_PWR_MEL, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
		/* gpio reset init */
		rc = gpio_request(MSM_8960_TS_PWR_MEL, "TOUCH_PANEL_PWR");
		if (unlikely(rc < 0))
			pr_err("%s not able to get gpio\n", __func__);
