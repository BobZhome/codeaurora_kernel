...
/*Modified according to Hitachi Spec. kiran.kanneganti@lge.com*/		
/*Initially power on IOVCC. Turn on VCC and VCI after IOVCC becomes stable*/	
/*Turn On VCC after IOVCC*/
#if 0		
...
/*Modified code according to hitachi Spec. kiran.kanneganti@lge.com*/
/*It reduces LCD on delay while wake up*/
/* After Power On reset pin needs to maintained LOW for min 1msec. Here for safety added 2 msecs. */
#if 0		
...
/*Modified code according to hitachi Spec. kiran.kanneganti@lge.com*/
/*It reduces LCD on delay while wake up*/
/*After reset need to wait more than 5msecs to start dsi clocks & sending commands. For safety added 10 msecs */		
		mdelay(10);
#if 0		
		mdelay(50);
#endif
...
/*Modified according to hitachi Spec. kiran.kanneganti@lge.com*/
/*It reduces LCD on delay while wake up*/
/* For exit sleep 6 ~ 7 frames waiting is enough. ( 7 * 16.6msec = 117 msec.). Reduced 10msec & made it 120 */
#if 0	
...
/*Modified according to hitachi Spec. kiran.kanneganti@lge.com*/
/*It reduces LCD on delay while wake up*/
/*for display on 1 frame waiting is enough. ( 16.6 msec). reduced 30 msec & made it 20 */
#if 0
...
static struct dsi_cmd_desc hitachi_power_off_set[] = {
	...
	/*kiran.kanneganti@lge.com. Will be modified after checkig with hitachi */	
	/* With 70 delay there is lcd distortion in lcd off seequence with back light on.*/	
    /*  START :  lx_kr enter_sleep 0 -> 70 -> 0  */
	{DTYPE_DCS_WRITE, 1, 0, 0, 0, sizeof(enter_sleep), enter_sleep}
    /*  END :  lx_kr enter_sleep 0 -> 70 -> 0 */
};
...
/*Add seperate buffers according to gamma tuning application*/
struct tuning_buff_write{
	char buf[TUNING_REGSIZE];
	int idx;
	int size;
};

struct tuning_buff_read{
...
{
	struct tuning_buff_read *rbuf = (struct tuning_buff_read *)tmp;
	int i;
...
{
	char *buff;
	struct tuning_buff_write *wbuf = (struct tuning_buff_write *)tmp;
	int i = wbuf->idx;
	int j = 0;
	...
	printk("LCD Reg Write Index %d, Size %d \n",i,wbuf->size);
	...
	for(j = 0; j < hitachi_power_on_set[i+4].dlen; j++ )
		printk("Reg values after tuning Val:%d Index:%d \n",hitachi_power_on_set[i+4].payload[j],j);
...
#ifndef CONFIG_MACH_MSM8960_L1m
#ifdef CONFIG_MSM_BUS_SCALING
...
extern unsigned long hidden_iova;
...
static struct backlight_platform_data lm3530_data = {
	...
	*/
	...
	.min_brightness = 0x54,//0x55,//0x56,//0x58,
	.max_brightness = 0x76,//0x75,//0x76,//0x78,
