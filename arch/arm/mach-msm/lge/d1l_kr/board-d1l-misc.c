...
static int vibrator_pwm_set(int enable, int amp, int n_value)
{
	...
	ERR_MSG("enable=%d, amp=%d, n_value=%d\n", enable, amp, n_value);
...
static int mhl_power_onoff(bool on, bool pm_ctrl)
{
	...
	if (pm_ctrl && lge_get_board_revno() >= HW_REV_B)
		return -EPERM;
...
static int vreg_set_ldo29(int on) // temp for Rev.A or Rev.B
{
	int rc = -EINVAL;
...
if((lge_get_board_revno() == HW_REV_A)||(lge_get_board_revno() == HW_REV_B)){
		vreg_set_ldo29(1);  // Temp for D1L_KR Rev.A or Rev.B
}
}
