...
#include <linux/input/qt602240_ts.h>
...
int synaptics_t1320_power_on(int on)
{
	...
	if(maker_id_val)
	{
		gpio_tlmm_config(GPIO_CFG(TS_POWER_ENABLE_FOR_ATMEL, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
		rc = 	gpio_request(TS_POWER_ENABLE_FOR_ATMEL , "atmel_touch_power_en");
		if (unlikely(rc < 0))
	...
	if(on)
	{
		...
		if(maker_id_val)
		{
			gpio_set_value(TS_POWER_ENABLE_FOR_ATMEL, 1);
		}else
		{
	...
	else
	{
		...
		if(maker_id_val)
			gpio_set_value(TS_POWER_ENABLE_FOR_ATMEL, 0);
	...
	if(maker_id_val)
		gpio_free(TS_POWER_ENABLE_FOR_ATMEL);
...
static struct qt602240_platform_data lge_atmel_ts_pdata = {
	.x_line		= 19,
	.y_line		= 11,
	.x_size		= 800,
	.y_size		= 480,
...
static struct i2c_board_info msm_i2c_atmel_ts_info[] = {
       {
               I2C_BOARD_INFO(LGE_TOUCH_ATMEL_NAME, LGE_TOUCH_ATMEL_I2C_SLAVE_ADDR),
               .platform_data = &lge_atmel_ts_pdata,
               .irq = MSM_GPIO_TO_INT(TS_GPIO_IRQ),
       }
};
...
static struct i2c_registry lge_vu2_touch_i2c_devices[] __initdata = {
		...
		},
		{
			I2C_SURF | I2C_FFA | I2C_FLUID,
			MSM_8960_GSBI3_QUP_I2C_BUS_ID,
			msm_i2c_atmel_ts_info,
			ARRAY_SIZE(msm_i2c_atmel_ts_info),
		},
};
