...
static struct melfas_tsi_platform_data melfas_ts_pdata_vzw_4key = {
	.gpio_scl = TS_GPIO_I2C_SCL,
	.gpio_sda = TS_GPIO_I2C_SDA,
	.gpio_ldo = MSM_8960_TS_PWR,
	.i2c_int_gpio = TS_GPIO_IRQ,
...
static struct i2c_board_info melfas_touch_panel_i2c_bdinfo[] = {
	...
	[2] = {
			I2C_BOARD_INFO(MELFAS_TS_NAME, MELFAS_TS_I2C_SLAVE_ADDR),
			.platform_data = &melfas_ts_pdata_vzw_4key,
			.irq = MSM_GPIO_TO_INT(TS_GPIO_IRQ),
	},
};
...
static void __init touch_init(void)
{
	...
	/* gpio init */
	rc = gpio_request(SYNAPTICS_T1320_TS_PWR, "TOUCH_PANEL_PWR");
	if (unlikely(rc < 0))
		pr_err("%s not able to get gpio\n", __func__);

	rc = gpio_request(SYNAPTICS_T1320_TS_MAKER_ID, "TOUCH_PANEL_MAKERID");

	if (unlikely(rc < 0))
		pr_err("%s not able to get gpio\n", __func__);

	/*  maker_id set 0 - synaptics t1320 4.3'' */
	gpio_direction_input(SYNAPTICS_T1320_TS_MAKER_ID);
}
...
#if defined(CONFIG_TOUCHSCREEN_ATMEL_MAXT224E) || defined(CONFIG_TOUCHSCREEN_MMS136)
	printk(KERN_INFO "MAKER_ID_VALUE is [ %d ]\n", maker_id_val);

	if (maker_id_val == 0) {
#if defined(CONFIG_USING_INNOTEK_PANEL_4_7)
		int revNo;
		revNo = (lge_get_board_revno() < HW_REV_C) ? lge_get_board_revno()-2 : 2 ;
		i2c_register_board_info(MSM_8960_GSBI3_QUP_I2C_BUS_ID,
			(&melfas_touch_panel_i2c_bdinfo[revNo]), 1);
#else
		i2c_register_board_info(MSM_8960_GSBI3_QUP_I2C_BUS_ID,
