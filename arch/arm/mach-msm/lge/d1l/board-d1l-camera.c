...
static struct msm_gpiomux_config msm8960_cam_common_configs_c[] = {
	{
		.gpio = GPIO_CAM_FLASH_EN, /* 1 */
		.settings = {
			[GPIOMUX_ACTIVE]    = &cam_settings[2],
...
void __init msm8960_init_cam(void)
{
	...
	if (lge_get_board_revno() < HW_REV_C)
		...
		msm_gpiomux_install(msm8960_cam_common_configs_c,
			ARRAY_SIZE(msm8960_cam_common_configs_c));
#elif defined(CONFIG_MACH_MSM8960_D1LV)
...
void __init lge_add_camera_devices(void)
{
	...
	if (lge_get_board_revno() >= HW_REV_C) {
	  ...
	  msm_camera_sensor_imx111_data.gpio_conf->cam_gpio_tbl_size = ARRAY_SIZE(msm_cam_gpio_2d_tbl_c);
	}
	*/
#endif
