...
// QCT PATCH START
void __init adjust_memory_overflow(void) 
{ 
	struct membank *mb = &meminfo.bank[meminfo.nr_banks - 1]; 
	//printk("[%s]1.  mb->start= 0x%lx, mb->size= 0x%lx\n",__FUNCTION__, (long unsigned int)mb->start, mb->size);
	if (mb->start + mb->size < mb->start) 
	{
		printk("[%s]2.pre:   mb->size= 0x%lx\n",__FUNCTION__, mb->size);
		mb->size -= (1 << PAGE_SHIFT);
		printk("[%s]3.next: mb->size= 0x%lx\n",__FUNCTION__, mb->size);	
	}
} 
// QCT PATCH END
