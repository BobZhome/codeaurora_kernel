...
static void set_dload_mode(int on)
{
	if (dload_mode_addr) {
		 ...
		 * hrset_enable = 0(userdebug), 1(user)
		 */
		if (on && hreset_enable && restart_mode != RESTART_DLOAD)
		{
			printk("%s: skip_dload_magic\n", __func__);
...
{
	if(restart_mode==0 || restart_mode!=0x6d630100)
		restart_mode = mode;
}
...
extern resource_size_t lge_add_info;
#endif
...
#ifdef CONFIG_MACH_LGE
	__raw_writel(__raw_readl(restart_reason), lge_add_info);
#endif
